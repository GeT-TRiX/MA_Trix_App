...
output$outputable <- DT::renderDataTable(DT::datatable(
  data()  ,extensions= extensions,  rownames = rownames , selection = selection ,
                ...
                {if (case == 3 && length(dupgenes()) != 0 ) { formatStyle( . ,'GeneName', color = styleEqual(unique(dupgenes()), rep('orange', length(unique(dupgenes())))))} else formatStyle(., NULL) }  , server = server)
